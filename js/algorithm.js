...
async function Our_algo(flag1) {
    ...
    // Add priority property to each process in the readyQueue
    for (let process of readyQueue) {
        process.priority = processQueue[process].priority// assign the priority level for each process
    }
    ...
    // Getting the initial processes into the process queue
    while (true) {
        if (readyQueue.length == 0 && processQueue.length == 0)
            break;

        for (let process of readyQueue) {
            if (process.arrival_time <= time && !processQueue.includes(process)) {
                processQueue.push(process);
            }
        }

        // Sort the processQueue based on priority
        ...
        let currentProcess = processQueue.shift(); // Get the process with highest priority

        // Process the current process for the time quanta or until it completes
        let remainingTime = currentProcess.burst_time - currentProcess.completed_time;
        let executionTime = Math.min(remainingTime, timeQuanta);

        if (ganttRoundRobin.length > 0 && ganttRoundRobin[ganttRoundRobin.length - 1].processId != currentProcess.processId) {
            ganttRoundRobin[ganttRoundRobin.length - 1].endTime = time;
            ganttRoundRobin.push({
        ...
        // Update the process details and metrics
        currentProcess.completed_time += executionTime;
        currentProcess.turnaround_time = time + executionTime - currentProcess.arrival_time;
        currentProcess.waiting_time = currentProcess.turnaround_time - currentProcess.burst_time;

        // Update the time and add the process back to the processQueue if it's not completed
        time += executionTime;
        if (currentProcess.completed_time < currentProcess.burst_time) {
            processQueue.push(currentProcess);
        }
    }

    // Calculate the remaining metrics (completionTime, turnAroundRR, responseRR, waitingRR) based on the completed processes
